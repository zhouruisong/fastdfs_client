...
import (
	...
	logger "github.com/zhouruisong/go-level-logger"
)
...
func NewFdfsClient(trackerlist []string, minConns, maxConns int) (*FdfsClient, error) {
...
func (this *FdfsClient) UploadByFilename(filename string) (*UploadFileResponse, error) {
	if err := fdfsCheckFile(filename); err != nil {
		logger.Errorf("fdfsCheckFile failed, filename:%+v,err:%+v", filename, err)
